...
.container .checkmark:after {
  ...
  transform: rotate(45deg);
   ...
   <body class="clinic_version" style="background-image: url('images/clinic_01.jpg');background-repeat: no-repeat;background-size: 100%" >
      <!-- LOADER -->
     <!--  <div id="preloader" >
               ...
               <nav class="main-menu" >
                  ...
                  <div id="navbar" class="navbar-collapse collapse" >
                     <ul class="nav navbar-nav" dir="rtl">
                        ...
                        <li><a data-scroll href="all_booking_reports.php">all booking reports</a></li>
      ...
      <div>
                  ...
                  <form method="post" action="booking_report_under_condision.php" style="direction: ltr;">
                              <select name="x"  onchange="this.form.submit()" style=" color: gray;"  > 
         <option selected disabled>choose specialization</option>
         <?php
       

         $sql="SELECT DISTINCT drspecialization from register";
         $res=$con->query($sql);
...
?>
  <!-- ////////////////////////////////////////////////////////////////  -->                   
         ...
         $sql="SELECT DISTINCT drdegree, drspecialization from register where drspecialization='$specialization'";
         $res=$con->query($sql);
         while($row=$res->fetch_assoc())
         {
          $special=$row['drspecialization'];
           $degree=$row['drdegree'];
...
?>
         ...
         $sql="SELECT DISTINCT drname from register where drspecialization='$specialization' and  drdegree='$degree'  ";
         $res=$con->query($sql);
         while($row=$res->fetch_assoc())
         {
           $name=$row['drname'];
    ...
    if(isset($_POST['report']))
      {
       // $_POST['Search'];
        ...
        // $_SESSION['date']=$date;
         $name=$_POST['name'];
      if ($name!=" "&& $date!="") {
           $sql="SELECT * FROM register where drname='$name' and bookdaydate='$date'" ;
         $res=$con->query($sql);
         bookingDisplay($res);
        ...
        if ($date==""&& $name!="") {
           ...
           $sql="SELECT * FROM register where drname='$name' " ;
         $res=$con->query($sql);
          bookingDisplay($res);
        // echo $name;
      }
      if($name==""&& $date!=""){
          ...
          $sql="SELECT * FROM register where  bookdaydate='$date'" ;
         $res=$con->query($sql);
         bookingDisplay($res);
            // $con->close();
         // echo $date;
